...
- Public packaged display names must stay channel-distinct: stable uses `OpenDesign`, beta uses `OpenDesign Beta`, prerelease uses `OpenDesign Prerelease`, and preview uses `OpenDesign Preview`. Persistent package identity remains `Open Design` plus the channel suffix: keep macOS app-bundle/executable names, Windows install/executable names, bundle IDs, registry keys, namespaces, and data roots on that existing identity. Do not ship beta, prerelease, or preview mac DMGs whose drag-install app bundle is `Open Design.app`.
