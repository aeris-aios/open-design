...
- TodoWrite has **no pinned card above the composer**. The task list appears exactly once, inside the turn's execution record, as `执行计划 · N 步` plus one drawer per step (`components/chat/ExecutionShell.tsx`, decisions D29 / B17 in `specs/current/chat-panel-next.md`). The old `PinnedTodoSlot` in `ChatPane.tsx` — and the auto-scroll plumbing that observed it — was removed when the new execution record landed; do not reintroduce it. `latestTodoWriteInputFromMessages` stays for other consumers, `unfinishedTodosFromEvents` (the daemon's own criterion) is untouched.
