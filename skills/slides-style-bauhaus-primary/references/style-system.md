# Bauhaus Primary Slides style system
...
**Start from `assets/template.html`. Replace content only. Never rewrite the design
...
1. Copy `assets/template.html`; keep all `<style>` blocks and the `<script>`
   verbatim.
