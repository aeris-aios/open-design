# Editorial Forest Slides style system
...
**Start from `assets/template.html` in this plugin folder. It is the locked seed: keep the inline `<deck-stage>` runtime script, the `:root` token block, and every layout skin verbatim — replace only slide content. Do not rewrite the design, do not introduce colors or fonts outside the spec below.**
