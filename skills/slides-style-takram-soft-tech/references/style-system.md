# Takram Soft Tech Slides style system
...
contract, and navigation runtime are locked by `assets/template.html`. **Start from
`assets/template.html`, replace content only — do not rewrite the design or the
...
2. **Copy `assets/template.html`**, retitle, then replace each section's content
