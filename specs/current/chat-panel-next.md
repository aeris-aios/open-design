...
| D46 | **组件 3 思考块的滚动形态要做**:运行中推理文字自己往上走。落点仍在执行记录壳内(D29 不变),即壳内的思考正文区限高 + 自动贴底 | 用户 2026-08-25:「做」。壳内 vs 独立块这一层未再细说,按 D29 落在壳内;若设计要求独立块再改 |
...
|---|---|---|---|---|---|
| ~~#41 #42~~ | ~~2~~ | 无框行、行间无缝、不留内外距 | ~~1px 边框 + 渐变底 + 10/12 内距~~ → **已按稿子改**(`.nexts` 那三条) | **容器的样子不需要等裁决,已清零**;还卡着的只有**内容**:契约与事件流里没有「本轮相关建议」字段 | T12(只剩内容那一半) |
...
**剩下 5 条拍完 + 补一个事件字段,这 84 格就能真的到 0。** 在那之前,把它们改成 0 只有两种办法:
...
| T12 | **组件 16 下一步引导没有数据源**:现在渲染的是固定工具目录,全仓没有任何「按本轮内容生成的建议」字段。稿子画的是 3 条跟本轮相关的建议 | 改 system prompt 让 agent 产出建议 / daemon 侧生成 / 维持固定目录(与稿子不一致) | 产品 | 组件 16 全部格子 |
...
| 组件 11「执行中 · 终端实时追加」**做不出来** | 镜像陈列页 #6 | 两层原因:①D3 定的是调用跑完才落行;②事件流里没有增量输出通道 —— `tool_result` 是终端内容的唯一载体,命令跑完才一次性到达。要做得让 daemon 新增一条工具进度事件 | 陈列页 #6 出格但不画实体,写明为什么;待产品/设计裁决要不要为它加事件 |
...
**接入前必须先解掉的**:T11(已回答形态)、T12(下一步引导数据源)、T19(回合状态行去重)——
这三条都会改变同屏信息量,等接进去再改等于返工。T18(S9 与 B17 二选一)决定 ④ 要不要保留胶囊。
