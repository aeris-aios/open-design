...
- `packages/preview-runtime`: shared pure TypeScript builders for scripts injected into artifact documents, plus browser-side helpers used by those scripts. Keep transport/session orchestration in the apps and keep each injected capability independently activatable.
