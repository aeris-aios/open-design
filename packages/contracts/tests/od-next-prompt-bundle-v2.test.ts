...
describe('OD Next canonical Prompt Bundle v2', () => {
  it('omits the task_type_skill slot when the task type ships no authored rule card', () => {
    const withoutTaskSkill = bundle({
      sessionSkills: {
        generalOrchestrationSkill: {
          skillName: 'general_orchestration',
          body: '# OD Next General Orchestration v2.0.0',
      ...
      taskMetadata: { taskType: 'image' },
    });
    const xml = serializeOdNextPromptBundleV2(withoutTaskSkill);
    ...
    expect(parsed.sessionSkills.taskTypeSkill).toBeUndefined();
