...
export type DesktopShowInput = { deeplinkUrl?: string };
export type DesktopAcceptedResult = { accepted: true };

type Method<Input, Output> = Readonly<{ input: Input; output: Output }>;
type EmptyInput = Record<string, never>;

/** Product-owned desktop capabilities transported by the generic Sidecar control plane. */
export type DesktopSidecarMethods = {
  click: Method<DesktopClickInput, DesktopClickResult>;
  console: Method<EmptyInput, DesktopConsoleResult>;
  eval: Method<DesktopEvalInput, DesktopEvalResult>;
  exportArtifact: Method<DesktopExportArtifactInput, DesktopExportArtifactResult>;
  exportPdf: Method<DesktopExportPdfInput, DesktopExportPdfResult>;
  renderSlides: Method<DesktopRenderSlidesInput, DesktopRenderSlidesResult>;
  screenshot: Method<DesktopScreenshotInput, DesktopScreenshotResult>;
  show: Method<DesktopShowInput, DesktopAcceptedResult>;
  status: Method<EmptyInput, DesktopStatusSnapshot>;
  update: Method<DesktopUpdateInput, DesktopUpdateResult>;
};
...
/** The sole runtime validators for desktop capability inputs. */
export const DESKTOP_SIDECAR_INPUTS = Object.freeze({
  click: z.object({ selector: z.string().min(1) }).strict(),
  console: z.object({}).strict(),
  eval: z.object({ expression: z.string().min(1) }).strict(),
  exportArtifact: z.object({
    baseHref: z.string().min(1).optional(),
    deck: z.boolean(),
  ...
  renderSlides: z.object({
    ...
    outputDir: absolutePath.optional(),
    pageImageFormat: z.enum(["png", "jpeg"]).optional(),
    paginate: z.boolean().optional(),
    stitch: z.boolean().optional(),
    width: optionalPositiveNumber,
  }).strict(),
  screenshot: z.object({ path: z.string().min(1) }).strict(),
  show: z.object({ deeplinkUrl: z.string().startsWith("opendesign://").optional() }).strict(),
  status: z.object({}).strict(),
  update: z.object({ action: z.enum([
    DESKTOP_UPDATE_ACTIONS.CHECK,
    DESKTOP_UPDATE_ACTIONS.CLEAR_CACHE,
    DESKTOP_UPDATE_ACTIONS.DOWNLOAD,
    DESKTOP_UPDATE_ACTIONS.INSTALL,
    DESKTOP_UPDATE_ACTIONS.STATUS,
  ]) }).strict(),
});
