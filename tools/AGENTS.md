...
- Orchestration layers must consume the public atomic control API from `@open-design/sidecar` and product DTOs from `@open-design/contracts` / `@open-design/host`.
- Do not hand-build transport env, process-scan regexes, runtime tokens, process roles, or a second namespace/source identity in tools or packaged launchers.
