...
test('header removes the promotional community CTA and keeps community navigation', () => {
  for (const locale of ['zh', 'zh-tw', 'en'] as const) {
    const html = render(locale);
    assert.doesNotMatch(html, /data-community-cta|nav-community|kokiai\.net\/community/);
    assert.match(html, /<span class="dropdown-name">Discord<\/span>/);
    assert.match(html, /aria-label="X"/);
  }

  assert.doesNotMatch(headerSource, /FEISHU|Feishu|飞书|飛書|data-community/);
  assert.doesNotMatch(stylesSource, /nav-community|nav-social-badge/);
});
