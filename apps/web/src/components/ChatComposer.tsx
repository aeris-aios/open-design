...
/* Running glyph: `thinking-orbs`' solving orb — the bands of a dotted sphere
   scramble in quarter turns, then click back. It draws to a <canvas> and parks
   itself when the tab is hidden or the element scrolls out of view, and
   honours prefers-reduced-motion by holding a single frame.

   `size` takes only 20 or 64 (the two are separately tuned designs, not a
   scale factor), so 20 it is — the mark's own ink box is ~14px, and the extra
   6px still clear the 32px disc.

   `theme` must be PINNED dark rather than left on `auto`: auto reads the host
   app's light/dark, but this disc is #202020 in BOTH app themes, so under a
   light app the orb would paint its dark ink onto the dark disc and vanish.
   Dark = light ink, which the CSS filter on `.composer-run-glyph` then carries
   to the mark's green. */
