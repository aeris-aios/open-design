...
// together the smaller pieces (HomeHero, PluginsHomeSection). Replaces the
// older left-side `PluginLoopHome`
...
/**
 * Seed a composer's prompt — used when another surface hands the user a
 * starting prompt (e.g. Community's "使用提示词"/Prompt button). Writes the
 * draft key (covers a true cold mount) AND dispatches a live event (covers the
 * common case where `HomeView` is already mounted and just gets toggled
 * visible — see the `HOME_COMPOSER_SEED_EVENT` note above).
 *
 * `target` names WHICH composer is being handed the prompt, because there can
 * be two on screen: Home's page one and a docked one (community). Only the
 * page composer owns the persisted draft, so a dock seed is live-event only —
 * writing the key would leave the prompt in Home's composer as well.
 */
...
export function HomeView({
  ...
  // Keep the team catalog subscription warm for project grids reached from
  // the rail. Home itself no longer renders a recent-projects grid.
  useTeamProjects();
  ...
  async function usePlugin(
    ...
    options?: {
      ...
      // feel instant; project creation resolves the snapshot transactionally.
      deferApply?: boolean;
  ...
  // the composer carries no pill. This used to silently bind the Slide deck
  ...
  function useExamplePlugin(record: InstalledPluginRecord, chipId: string, promptText: string) {
    ...
    // deferApply mirrors the chip rail: bind now, let project creation resolve
    // the snapshot, so a preset click stays instant and doesn't fire an /apply
    // roundtrip per card. The chip is already
  ...
  // seed its text, then defer the create to submit() (which builds the same
  // payload as the manual flow; project creation resolves the snapshot).
  ...
  async function submit() {
    ...
    // project-create handoff so the click count reflects user intent even when
    // the run is rejected (for example, missing inputs or create failure). The
    ...
    setError(null);
    // This is an internal single-flight lock only. The destination Chat frame
    // owns progress, and onSubmit is reached without a Home-side network wait.
    setSending(true);
    try {
      ...
      const submittedPluginInputs = submittedActive
        ...
        : defaultInputs;
      // Reuse a ready snapshot only when it describes the exact inputs being
      // sent. Deferred type chips and deferral-stripped media inputs pass no
      // snapshot: POST /api/projects already resolves pluginId + pluginInputs
      // in the same transaction as project/conversation creation. Keeping that
      // work there lets the optimistic Chat route start synchronously.
      const appliedPluginSnapshotId =
        submittedActive
      ...
      // Directory validity is authoritative at POST /api/projects, which
      // validates metadata.linkedDirs before inserting anything. Avoiding a
      // duplicate /api/dir-exists preflight keeps every Home send on the same
      // synchronous optimistic handoff path.
      const contextLinkedDirs = workspaceContextLinkedDirs(contextWorkspaceItems);
  ...
  return (
    ...
    >
      ...
      {/* Home stays focused on the composer and starter examples. Existing
          projects remain available from the rail's 个人项目 destination. */}
