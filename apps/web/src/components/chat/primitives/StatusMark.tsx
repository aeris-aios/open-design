...
export function StatusMark({ status, index }: StatusMarkProps): ReactElement {
  ...
  if (status === 'ok') {
    // 图当底,里面不放东西 —— 勾是图的一部分
    return <span className={`${styles.mark} ${styles.ok}`} aria-label="已完成" role="img" />;
  }

  if (status === 'fail') {
    return (
      <span className={`${styles.mark} ${styles.fail}`} aria-label="失败" role="img">
  ...
  if (status === 'running') {
     ...
     */
    return (
      <span className={`${styles.mark} ${styles.run}`} role="img" aria-label="进行中">
  ...
  if (status === 'skip') {
    ...
    return <span className={`${styles.mark} ${styles.ok}`} aria-label="已取消" role="img" />;
  }

  return <span className={`${styles.mark} ${styles.pending}`} aria-label="未开始" role="img" />;
}
