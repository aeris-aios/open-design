...
describe('DeepSeek V4 Pro campaign', () => {
  ...
  it('keeps the promotion attached only to the Pro model', () => {
    expect(isDeepSeekV4ProCampaignModel('deepseek-v4-pro')).toBe(true);
    expect(isDeepSeekV4ProCampaignModel(' DeepSeek-V4-Pro ')).toBe(true);
    expect(isDeepSeekV4ProCampaignModel('deepseek-v4-flash')).toBe(false);
  ...
  it('keeps the fixed window out of the primary headline and badge', () => {
    expect(DEEPSEEK_V4_PRO_CAMPAIGN.headline).toBe('这次，更聪明的模型别省着用。');
    expect(DEEPSEEK_V4_PRO_CAMPAIGN.badge).toBe('无限使用');
    expect(DEEPSEEK_V4_PRO_CAMPAIGN.timing).toContain('8 月 14 日至 8 月 21 日');
    ...
    expect(DEEPSEEK_V4_PRO_CAMPAIGN.window.startAt).toBe('2026-08-14T20:00:00+08:00');
    expect(DEEPSEEK_V4_PRO_CAMPAIGN.window.endAtExclusive).toBe('2026-08-21T20:00:00+08:00');
    expect(DEEPSEEK_V4_PRO_CAMPAIGN.window.label).toBe('8 月 14 日—8 月 21 日');
  });
  ...
  it('keeps the campaign promise stable while routing actions by entitlement', () => {
    ...
    expect(DEEPSEEK_V4_PRO_CAMPAIGN.unpaid.tooltip).toContain('8 月 21 日');
  ...
  it('shows a shared live countdown in both paid and unpaid campaign modals', () => {
    ...
    expect(formatDeepSeekV4ProCampaignCountdown(start)).toBe('7天 00:00:00');
    expect(formatDeepSeekV4ProCampaignCountdown(end)).toBe('活动已结束');
    expect(formatDeepSeekV4ProCampaignMockRemaining(7 * 24 * 60 * 60 * 1000)).toBe(
      '7天 00:00:00',
    );
