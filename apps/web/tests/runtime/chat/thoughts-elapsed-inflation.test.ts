...
describe('口子一 · 顺序不该改变结论', () => {
  ...
  /**
   * 同一条不对称,推理被**切成几截**的形态:中间夹一个「发出去了但结果还没回来」的
   * 调用(D3:不落行),推理仍是同一段文字,却分两次结账。
   *
   * 前一截该被拒(它前面躺着正文),后一截干净 —— 只把后一截加起来得到的是
   * **偏小的假数**,同样违反 §2.2b。整段作废才对。
   */
  it('`正文 → 思考(被切成两截) → 工具`:整段作废,不报剩下那一截', () => {
    const shell = sole({
      ...TURN,
      events: [
        ...call('t1', 1_000_000, 1_002_000),
    ...
    expect(cells.length, '两截仍是同一段文字,收成一格').toBe(1);
    expect(cells[0]!.elapsedMs, '18s 是把正文那 8 秒也算进去了').not.toBe(18_000);
    expect(cells[0]!.elapsedMs, '10s 是只算干净那一截的偏小假数').not.toBe(10_000);
