...
const connectSidecar = vi.hoisted(() => vi.fn());
...
describe("resolveDaemonUrl", () => {
  ...
  beforeEach(() => {
    connectSidecar.mockRejectedValue(new Error("no inherited control capability"));
  });
  ...
  it("discovers the live daemon URL through the inherited control capability", async () => {
    ...
    connectSidecar.mockResolvedValue({ call });

    await expect(resolveDaemonUrl({ env: {}, timeoutMs: 1000 }))
      .resolves.toBe("http://127.0.0.1:54321");
    expect(call).toHaveBeenCalledWith("status", {}, { timeoutMs: 1000 });
  });
});
