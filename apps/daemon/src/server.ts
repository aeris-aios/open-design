...
export async function startServer({
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    let userRequestPrompt = isOdNextRequestStage
      ? resolveOdNextRequestUserPrompt({
          message,
          currentPrompt,
          hasCurrentPrompt: hasExplicitCurrentPrompt,
    ...
    } = titleGenerationForResumeState(agentResumePromptPolicy.skipTranscript);
    ...
    const requestInputPendingFact = isOdNextRequestStage
      ...
      : '';
    const composeLegacyPayload = (requestText: string) => {
      const includeStableForPayload = isOdNextRequestStage || includeStableInstructions;
      return composeChatAgentTextPayload({
        ...
        requestOrStageText: requestText,
    ...
    // OD Next continuations are bound to their locked native session and exact
    // persisted input. Its coordinator owns any future context rollover; the
    // legacy resume path below may safely cold-seed a compacted transcript.
    const sessionContextBudget = agentResumePromptPolicy.skipTranscript && !strategyTaskAtStart
      ? evaluateModelContextBudget({
          prompt: composedResult.composedPrompt,
          modelId: safeModel,
    ...
    if (sessionContextBudget?.action === 'rollover') {
      ...
      composedResult = composeLegacyPayload(userRequestPrompt);
    }
