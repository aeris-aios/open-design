...
Options:
  --daemon-url <url>   OpenDesign daemon HTTP base URL. Resolution
                       order: this flag, OD_DAEMON_URL, inherited Sidecar capability,
...
async function mintCliImportToken(baseDir) {
  let result;
  try {
    const daemon = await connectSidecar<DaemonSidecarMethods>();
    result = await daemon.call('mintImportToken', { baseDir }, { timeoutMs: 800 });
  } catch {
    return null;
  }
