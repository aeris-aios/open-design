...
export type RuntimeVersionPolicy = {
  /**
   * Exact version strings accepted by this Open Design build. Keep this
   * opt-in: most existing adapters historically remain usable when their
   * version probe is absent or unsupported.
   */
  supportedVersions: string[];
  /** Fail closed when the version probe fails or prints no usable version. */
  requireVersion: true;
};
