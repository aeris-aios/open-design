...
// silently misses the inherited Sidecar capability.
//
// Side effects (the fs.existsSync probes, process.execPath, the
// ELECTRON_RUN_AS_NODE env read, OD_DATA_DIR resolution, Sidecar capability
// detection) all stay in the caller. This module is intentionally pure
// and free of generic transport details so it can be unit-tested
...
export interface BuildMcpInstallPayloadInputs {
   ...
   *  spawned `od mcp` should discover the live URL through its
   *  inherited capability instead of a baked --daemon-url. */
  isSidecarMode: boolean;
  /** Already-filtered opaque Sidecar env entries the
...
export function buildMcpInstallPayload(
  ...
  // the live URL through the inherited capability on every spawn, surviving
