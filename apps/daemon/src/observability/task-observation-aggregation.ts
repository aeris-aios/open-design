...
const CONTEXT_BUDGET_ACTIONS = new Set([
  'unmeasured',
  'within_budget',
  'blocked',
  'rollover',
]);
const CONTEXT_BUDGET_SOURCES = new Set([
  'model_metadata',
  'known_model_family',
  'unknown',
]);

function safeTaskObservationContextBudgetMetadata(
  observation: NormalizedAgentObservationV1,
): Record<string, string | number> {
  if (observation.kind !== 'task_run') return {};
  const attributes = observation.attributes ?? {};
  const metadata: Record<string, string | number> = {};
  const action = attributes.contextBudgetAction;
  const source = attributes.contextBudgetSource;
  if (typeof action === 'string' && CONTEXT_BUDGET_ACTIONS.has(action)) {
    metadata.contextBudgetAction = action;
  }
  if (typeof source === 'string' && CONTEXT_BUDGET_SOURCES.has(source)) {
    metadata.contextBudgetSource = source;
  }
  for (const key of [
    ...
    'projectedSessionInputTokens',
    'rolloverThresholdTokens',
  ...
  if (observation.kind === 'task_run') {
    ...
    return {
      ...
      metadata: {
        ...
        ...safeTaskObservationContextBudgetMetadata(observation),
