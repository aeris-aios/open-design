...
test('[P1] home design card deletion supports cancel and confirm flows', async ({ page }) => {
  ...
  // Move to the designs route through the SPA so the open-project tab stays
  // alive. A full page.goto('/projects') rebuilds chrome from persisted scope
  // state and cannot witness the same-session removal event. The refreshed
  // rail currently exposes no /projects destination, so dispatch route truth
  // directly instead of pretending there is a user-facing control.
  await page.evaluate(() => {
